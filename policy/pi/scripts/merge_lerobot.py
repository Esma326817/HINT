...
python merge_lerobot.py \
  --src_paths \
    "/dataset/robot/real_world/piper/lerobot/classify/classify_stage_rendered_merged_v1_v2_bak" \
    "/dataset/robot/real_world/piper/lerobot/classify/classify_v3_highlighting_480x640" \
  --tgt_path "/dataset/robot/real_world/piper/lerobot/classify/classify_480_640_render_mixed" \
  --repo_id "piper/classify/classify_480_640_render_mixed"
"""
...
def strip_video_codec_info(features: Dict[str, Any] | None) -> Dict[str, Any]:
    """Return a copy of features suitable for schema equality checks.

    After episodes are saved, LeRobot may fill ``features[k]["info"]`` from **merged**
    episode-0 videos; another source repo still has **its own** per-file codec metadata.
    Those differ while dtype/shape/names are the same — strip ``info`` under ``dtype==video``.
    """
    if not features:
        return {}
    out = copy.deepcopy(features)
    for _key, spec in out.items():
...
def ensure_features_compatible(tgt_info: Dict[str, Any], src_info: Dict[str, Any], src_name: str, force: bool, warnings: List[str]):
    ...
    # Schema-level features match: ignore per-repo video "info" (see strip_video_codec_info)
    if "features" in tgt_info and "features" in src_info:
        t_schema = strip_video_codec_info(tgt_info["features"])
        s_schema = strip_video_codec_info(src_info["features"])
        if t_schema != s_schema:
            msg = (
                f"Conflict features dict (schema) between target and source {src_name}. "
                f"If only video 'info' differed, this should be rare after strip; check dtype/shape keys."
            )
            if force:
                warnings.append("FORCED: " + msg)
            else:
