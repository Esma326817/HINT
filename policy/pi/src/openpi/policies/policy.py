...
class Policy(BasePolicy):
    ...
    def infer(self, obs: dict, *, noise: np.ndarray | None = None) -> dict:  # type: ignore[misc]
        ...
        # Handle both dict return (with extra features) and plain array return.
        if isinstance(sample_result, dict):
            actions = sample_result["actions"]
            extra_outputs = {k: v for k, v in sample_result.items() if k != "actions"}
        else:
        ...
        # Add extra outputs (e.g. global_mean_feature) after output_transform
        # so they are not affected by action-specific transforms.
