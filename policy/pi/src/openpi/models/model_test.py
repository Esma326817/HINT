...
def test_pi0_grounding_attention_model():
    key = jax.random.key(0)
    config = pi0_config.Pi0Config(
    ...
    result = nnx_utils.module_jit(model.sample_actions)(key, obs, num_steps=1)
    assert result["actions"].shape == (1, model.action_horizon, model.action_dim)
