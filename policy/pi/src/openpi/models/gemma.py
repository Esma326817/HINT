...
class Attention(nn.Module):
    ...
    def __call__(
        ...
        attn_mask,
        kv_cache,
        attention_query_gate=None,
        attention_key_weights=None,
        attention_layer_gate=1.0,
    ):
        ...
        if self.attention_injection and attention_query_gate is not None and attention_key_weights is not None:
            if attention_query_gate.shape != (q.shape[0], q.shape[1]):
                raise ValueError(
                    f"Attention query gate has shape {attention_query_gate.shape}, expected {(q.shape[0], q.shape[1])}"
                )
            if attention_key_weights.shape != (q.shape[0], k.shape[1]):
                raise ValueError(
                    f"Attention key weights have shape {attention_key_weights.shape}, expected {(q.shape[0], k.shape[1])}"
                )
            if self.attention_alpha_mode == "learned_bounded":
                ratio = self.attention_alpha / self.attention_alpha_max
                beta_init = math.log(ratio / (1.0 - ratio))
                beta = self.param("attention_beta", nn.initializers.constant(beta_init), ())
                alpha = self.attention_alpha_max * jax.nn.sigmoid(beta)
            else:
                alpha = jnp.asarray(self.attention_alpha, dtype=jnp.float32)
            grounding_bias = (
                jnp.asarray(attention_layer_gate, dtype=jnp.float32)
                * alpha
                * jnp.asarray(attention_query_gate, dtype=jnp.float32)[:, None, None, :, None]
                * jnp.asarray(attention_key_weights, dtype=jnp.float32)[:, None, None, None, :]
            )
            logits = logits + grounding_bias
...
class Block(nn.Module):
    ...
    def __call__(
        ...
        attn = Attention(
            ...
            attention_alpha_mode=self.attention_alpha_mode,
            attention_alpha=self.attention_alpha,
            attention_alpha_max=self.attention_alpha_max,
            name="attn",
        )
...
def _make_attention_injection_layer_gates(depth: int, layer_numbers: Sequence[int]) -> jax.Array:
    """Maps one-based configured layer numbers to scan gates."""
    return jnp.asarray(
        [layer_number in layer_numbers for layer_number in range(1, depth + 1)],
        dtype=jnp.bool_,
    )
...
class Module(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        layer_gates = _make_attention_injection_layer_gates(
            self.configs[0].depth,
            self.attention_injection_layers,
        )
    ...
    def init(self, use_adarms: Sequence[bool]):
        ...
        init_attention = jnp.ones((1, len(self.configs)), dtype=jnp.float32) if self.attention_injection else None
        self(
            [jnp.zeros((1, 1, c.width)) for c in self.configs],
            jnp.zeros((1, len(self.configs)), dtype=jnp.int32),
            jnp.zeros((1, len(self.configs), len(self.configs)), dtype=bool),
            adarms_cond=[jnp.zeros((1, c.width)) if u else None for u, c in zip(use_adarms, self.configs, strict=True)],
            attention_query_gate=init_attention,
            attention_key_weights=init_attention,
        )
